...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/" name="Classification">
        ...
        <!-- Common ML imports -->
        <xsl:if test="pipeline/Classification/preProcessing/step">
        ...
        <xsl:value-of select="pipeline/Classification/evaluation/metric"/><xsl:text>&#10;</xsl:text>

        <!-- Data Loading -->
        <xsl:text>&#10;# Data Loading&#10;</xsl:text>
        <xsl:text>data = pd.read_csv('</xsl:text><xsl:value-of select="pipeline/Classification/dataset/@path"/><xsl:text>')&#10;</xsl:text>

        <!-- Feature Engineering -->
        <xsl:text>features = [</xsl:text>
        <xsl:for-each select="pipeline/Classification/dataset/features/feature">
            '<xsl:value-of select="name"/>'<xsl:if test="position() != last()">, </xsl:if>
        </xsl:for-each>
        <xsl:text>]&#10;</xsl:text>
        <xsl:text>target = '</xsl:text><xsl:value-of select="pipeline/Classification/dataset/target/name"/><xsl:text>'&#10;</xsl:text>
        ...
        <xsl:if test="pipeline/Classification/preProcessing/step">
            ...
            <xsl:for-each select="pipeline/Classification/preProcessing/step">
                <xsl:choose>
        ...
        <xsl:choose>
            <xsl:when test="pipeline/@library='tensorflow'">
                ...
                <xsl:for-each select="pipeline/Classification/model/arch/layer">
                ...
                <xsl:text>        optimizer='</xsl:text><xsl:value-of select="pipeline/Classification/model/hyperparameters/optimizer"/><xsl:text>'</xsl:text>
                <xsl:text>,&#10;        loss='</xsl:text><xsl:value-of select="pipeline/Classification/model/hyperparameters/loss"/><xsl:text>'</xsl:text>
                <xsl:if test="pipeline/Classification/model/hyperparameters/metrics">
                    <xsl:text>,&#10;        metrics=[</xsl:text>
                    <xsl:for-each select="pipeline/Classification/model/hyperparameters/metrics/metric">
                ...
                <xsl:for-each select="pipeline/Classification/model/hyperparameters/param">
            ...
            <xsl:when test="pipeline/@library='pytorch'">
                ...
                <xsl:text>    optimizer=optim.</xsl:text><xsl:value-of select="pipeline/Classification/model/hyperparameters/optimizer"/><xsl:text>,&#10;</xsl:text>
                <xsl:text>    criterion=nn.</xsl:text><xsl:value-of select="pipeline/Classification/model/hyperparameters/loss"/><xsl:text>,&#10;</xsl:text>
                <xsl:text>    max_epochs=</xsl:text><xsl:value-of select="pipeline/Classification/model/hyperparameters/epochs"/><xsl:text>,&#10;</xsl:text>
                <xsl:text>    batch_size=</xsl:text><xsl:value-of select="pipeline/Classification/model/hyperparameters/batch_size"/><xsl:text>&#10;</xsl:text>
                <xsl:text>)&#10;&#10;</xsl:text>
            </xsl:when>
        </xsl:choose>
        ...
        <xsl:value-of select="pipeline/Classification/data_split/test_size"/><xsl:text>, random_state=</xsl:text>
        <xsl:value-of select="pipeline/Classification/data_split/random_state"/><xsl:text>&#10;)&#10;&#10;</xsl:text>
        ...
        <xsl:text>print("</xsl:text><xsl:value-of select="pipeline/Classification/evaluation/metric"/><xsl:text>: ", </xsl:text>
        <xsl:value-of select="pipeline/Classification/evaluation/metric"/><xsl:text>(y_test, predictions))&#10;</xsl:text>
    </xsl:template>
